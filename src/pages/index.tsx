...
function Index() {
  ...
  return (
    <>
      <div className={styles.header}>{namesError ? <p>Failed to load pages</p> : names ? names.map((name) => <div className={styles.pages} onClick={() => setPageNum(name.pageNum)} key={name.id} >{name.name}</div>) : "Loading..."}</div>
      {pageError ? (
        <>
